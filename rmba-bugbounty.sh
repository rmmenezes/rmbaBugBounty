...
case $option in
    ...
    2)
        ...
        cat subfinder-output.txt | ~/go/bin/gau --blacklist md,jpg,jpeg,gif,css,tif,tiff,png,ttf,woff,woff2,ico,PNG --from 202201 --providers wayback --o gau-output.txt
        ...
        cat uro-output.txt | ~/go/bin/dalfox pipe --silence --format json -o dalfox-output.json
